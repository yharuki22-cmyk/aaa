...
def main(out_path="output/office_floor_plan.dxf"):
    ...
    # ================= 壁 =================
    # 外壁
    pl.wall_x(0, 0, 7_400, T_EXT, openings=[(2_200, 3_400), (4_700, 6_100)])
    pl.wall_x(12_000, 0, 7_400, T_EXT,
              openings=[(800, 2_400), (3_200, 4_800), (5_600, 6_800)])
    pl.wall_y(0, 0, 12_000, T_EXT, openings=[
        (2_700, 3_900),                       # 玄関両開き
        (4_700, 5_600), (7_000, 7_900), (10_000, 10_900),   # 事務室ドア
        (5_900, 6_700), (8_300, 9_700),       # 窓
    ])
    pl.wall_y(7_400, 0, 12_000, T_EXT, openings=[
        (2_700, 3_600), (4_700, 6_100), (7_600, 9_000), (10_700, 11_600)])
    # X2通りの壁 (事務室仕切り、両開きドア)
    ...
    # ================= 建具 =================
    # 窓 (壁開口に合わせる)
    for y1, y2 in [(2_200, 3_400), (4_700, 6_100)]:
        pl.window((0, y1), (0, y2), T_EXT)
    for y1, y2 in [(800, 2_400), (3_200, 4_800), (5_600, 6_800)]:
        pl.window((12_000, y1), (12_000, y2), T_EXT)
    for x1, x2 in [(5_900, 6_700), (8_300, 9_700)]:
        pl.window((x1, 0), (x2, 0), T_EXT)
    for x1, x2 in [(2_700, 3_600), (4_700, 6_100), (7_600, 9_000), (10_700, 11_600)]:
    ...
    pl.door((2_700, 0), (3_300, 0), side=1)            # 玄関 両開き
    pl.door((3_900, 0), (3_300, 0), side=-1)
    for x in (4_700, 7_000, 10_000):                   # 事務室 掃き出しドア
    ...
    # ================= 設備機器 =================
    # 男子便所: 大便器 (X1壁際) + 手洗い
    pl.rect(150, 6_430, 400, 6_830, "05設備機器")
    msp.add_circle(pl.p(650, 6_630), 220, dxfattribs={"layer": "05設備機器"})
    pl.line((400, 6_500), (650, 6_500), "05設備機器")
    pl.line((400, 6_760), (650, 6_760), "05設備機器")
    msp.add_circle(pl.p(500, 4_980), 180, dxfattribs={"layer": "05設備機器"})
    # 湯沸室: 流し台
    pl.rect(2_600, 6_900, 3_900, 7_290, "05設備機器")
    pl.rect(3_250, 6_980, 3_800, 7_210, "05設備機器")
